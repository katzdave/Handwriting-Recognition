...
(function() {
    ...
    function createCanvas(parent, width, height) {
        ...
        canvas.node.style.margin = '0 0.5em 0 0.5em';
    ...
    senderButton.onclick = function() {
        ...
        for (i = 0; i != allCanvases.length; ++i) {
            if (allCanvases[i].hasDrawn) {
                ...
                //alert(data.data[0] + " " + data.data[1] + " " + data.data[2] + " " + data.data[3] + " " + data.data[4] + " " + data.data[upperBound]);
            }
        }
        ...
        $.ajax({
            type: 'POST',
            data: JSON.stringify(toServerObj),
            success: alert('hi'),
    ...
    function reduceCanvasToFV(imageDataArr) {
        ...
        //128 length vector
        //alert(featureVector.length);
        //alert(featureVector.toString());
    ...
    testAjax.onclick = function() {
        ...
        //document.getElementById('status').innerHTML="sent to server!";
    };
    /*
